...
if (isset($Confirm)) {   // 確認按鈕
	...
	$sqlcmd = "SELECT count(*) AS c FROM activities WHERE name='$Activityname' AND soft_delete='0' AND id!='$ActivityID'";
	$Contacts = querydb($sqlcmd, $db_conn);
	$C=$Contacts[0]['c'];
	if($C>0) $ErrMsg = '此活動已存在\n';
	
    if (empty($ErrMsg)) {   // 資料經初步檢核沒問題
        $sqlcmd="UPDATE activities SET name='$Activityname',soft_delete='$Status',start_date='$start_date',end_date='$end_date' WHERE id='$ActivityID'";
		...
		if ( $_FILES["upfile"]["size"] > 0 ) 
        {
			...
			if ($typeok)
			{
				$saveto = "/images/activities/".$ActivityID.".png";
...
// 此處是在contactlist.php點選後進到這支程式，因此要由資料表將欲編輯的資料列調出
	$sqlcmd = "SELECT * FROM activities WHERE id=$pid";
